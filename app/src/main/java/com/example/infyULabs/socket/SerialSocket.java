package com.example.infyULabs.socket;
...
public class SerialSocket implements Runnable {
    ...
    ScanBeanFromDevice scanBeanFromDevice = new ScanBeanFromDevice();

    public SerialSocket(Context context, BluetoothDevice device) {
        Log.e("serial socket", device + "");
    ...
    public SerialSocket(Context context, BluetoothSocket socket) {
        this.context = context;
        this.socket = socket;
    }
    ...
    public void run() {
        // connect & read
        try {
            Log.e("run method", device + "");
            if (socket == null) {
                socket = device.createRfcommSocketToServiceRecord(BLUETOOTH_SPP);
            }
            Log.e("connection socket", socket + "");
            socket.connect();
            scanBeanFromDevice.setBluetoothSocket(socket);
            if (listener != null)
