...
import com.github.mikephil.chartingNIR.bufferNIR.CircleBuffer;
import com.github.mikephil.chartingNIR.bufferNIR.LineBuffer;
...
public class LineChartRenderer extends LineScatterCandleRadarRenderer {
    ...
    public void initBuffers() {
        ...
        LineDataNIR lineDataNIR = mChart.getLineData();
        mLineBuffers = new LineBuffer[lineDataNIR.getDataSetCount()];
        mCircleBuffers = new CircleBuffer[lineDataNIR.getDataSetCount()];

        for (int i = 0; i < mLineBuffers.length; i++) {
            LineDataSetNIR set = lineDataNIR.getDataSetByIndex(i);
    ...
    public void drawData(Canvas c) {
        ...
        for (LineDataSetNIR set : lineDataNIR.getDataSets()) {
    ...
    protected void drawDataSet(Canvas c, LineDataSetNIR dataSet) {
     ...
     * @param entries
     */
    protected void drawCubic(Canvas c, LineDataSetNIR dataSet, List<EntryNIR> entries) {

        Transformer trans = mChart.getTransformer(dataSet.getAxisDependency());

        EntryNIR entryNIRFrom = dataSet.getEntryForXIndex(mMinX);
        EntryNIR entryNIRTo = dataSet.getEntryForXIndex(mMaxX);

        int diff = (entryNIRFrom == entryNIRTo) ? 1 : 0;
        int minx = Math.max(dataSet.getEntryPosition(entryNIRFrom) - diff, 0);
        int maxx = Math.min(Math.max(
                minx + 2, dataSet.getEntryPosition(entryNIRTo) + 1), entries.size());
        ...
        if (size - minx >= 2) {
            ...
            EntryNIR prevPrev = entries.get(minx);
            EntryNIR prev = entries.get(minx);
            EntryNIR cur = entries.get(minx);
            EntryNIR next = entries.get(minx + 1);
        ...
        if (dataSet.isDrawFilledEnabled()) {
            ...
            drawCubicFill(mBitmapCanvas, dataSet, cubicFillPath, trans,
                    entryNIRFrom.getXIndex(), entryNIRFrom.getXIndex() + size);
        }
    ...
    protected void drawCubicFill(Canvas c, LineDataSetNIR dataSet, Path spline, Transformer trans,
                                 int from, int to) {
     ...
     * @param entries
     */
    protected void drawLinear(Canvas c, LineDataSetNIR dataSet, List<EntryNIR> entries) {
    ...
    protected void drawLinearFill(Canvas c, LineDataSetNIR dataSet, List<EntryNIR> entries, int minx,
                                  int maxx,
                                  Transformer trans) {
     ...
     * @return
     */
    private Path generateFilledPath(List<EntryNIR> entries, float fillMin, int from, int to) {
    ...
    public void drawValues(Canvas c) {
        ...
        if (mChart.getLineData().getYValCount() < mChart.getMaxVisibleCount()
            ...
            List<LineDataSetNIR> dataSets = mChart.getLineData().getDataSets();

            for (int i = 0; i < dataSets.size(); i++) {

                LineDataSetNIR dataSet = dataSets.get(i);
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    EntryNIR entryNIR = entries.get(j / 2 + minx);

                    drawValue(c, dataSet.getValueFormatter(), entryNIR.getVal(), entryNIR, i, x,
                            y - valOffset);
                }
            }
        }
    ...
    protected void drawCircles(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            EntryNIR entryNIRFrom = dataSet.getEntryForXIndex((mMinX < 0) ? 0 : mMinX);
            EntryNIR entryNIRTo = dataSet.getEntryForXIndex(mMaxX);
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (int i = 0; i < indices.length; i++) {
            ...
            LineDataSetNIR set = mChart.getLineData().getDataSetByIndex(indices[i]
                    .getDataSetIndex());
    ...
    /**
     * Releases the drawing bitmap. This should be called when {@link LineChartNIRNIR#onDetachedFromWindow()}.
     */
