...
import com.github.mikephil.chartingNIR.dataNIR.BubbleDataSet;
import com.github.mikephil.chartingNIR.dataNIR.BubbleEntryNIR;
import com.github.mikephil.chartingNIR.dataNIR.EntryNIR;
import com.github.mikephil.chartingNIR.interfaces.BubbleDataProvider;
...
public class BubbleChartRenderer extends DataRenderer {
    ...
    protected void drawDataSet(Canvas c, BubbleDataSet dataSet) {
        ...
        List<BubbleEntryNIR> entries = dataSet.getYVals();

        EntryNIR entryNIRFrom = dataSet.getEntryForXIndex(mMinX);
        EntryNIR entryNIRTo = dataSet.getEntryForXIndex(mMaxX);

        int minx = Math.max(dataSet.getEntryPosition(entryNIRFrom), 0);
        int maxx = Math.min(dataSet.getEntryPosition(entryNIRTo) + 1, entries.size());
        ...
        for (int j = minx; j < maxx; j++) {
            ...
            final BubbleEntryNIR entry = entries.get(j);
    ...
    public void drawValues(Canvas c) {
        ...
        if (bubbleData.getYValCount() < (int) (Math.ceil((float) (mChart.getMaxVisibleCount())
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                final List<BubbleEntryNIR> entries = dataSet.getYVals();

                EntryNIR entryNIRFrom = dataSet.getEntryForXIndex(mMinX);
                EntryNIR entryNIRTo = dataSet.getEntryForXIndex(mMaxX);

                int minx = dataSet.getEntryPosition(entryNIRFrom);
                int maxx = Math.min(dataSet.getEntryPosition(entryNIRTo) + 1, dataSet.getEntryCount());
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    BubbleEntryNIR entry = entries.get(j / 2 + minx);
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (Highlight indice : indices) {
            ...
            final BubbleEntryNIR entry = (BubbleEntryNIR) bubbleData.getEntryForHighlight(indice);
