 ...
 */
public class CandleDataSet extends LineScatterCandleRadarDataSet<CandleEntryNIR> {
    ...
    public CandleDataSet(List<CandleEntryNIR> yVals, String label) {
        super(yVals, label);
    }

    @Override
    public DataSet<CandleEntryNIR> copy() {

        List<CandleEntryNIR> yVals = new ArrayList<CandleEntryNIR>();

        for (int i = 0; i < mYVals.size(); i++) {
            yVals.add(((CandleEntryNIR) mYVals.get(i)).copy());
        }
    ...
    protected void calcMinMax(int start, int end) {
        ...
        List<CandleEntryNIR> entries = mYVals;
