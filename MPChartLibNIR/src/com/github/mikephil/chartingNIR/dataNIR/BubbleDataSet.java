...
public class BubbleDataSet extends BarLineScatterCandleBubbleDataSet<BubbleEntryNIR> {
    ...
    public BubbleDataSet(List<BubbleEntryNIR> yVals, String label) {
        super(yVals, label);
    }
    ...
    protected void calcMinMax(int start, int end) {
        ...
        final List<BubbleEntryNIR> entries = getYVals();
        ...
        for (int i = start; i <= endValue; i++) {
            ...
            final BubbleEntryNIR entry = entries.get(i);
    ...
    @Override
    public DataSet<BubbleEntryNIR> copy() {

        List<BubbleEntryNIR> yVals = new ArrayList<BubbleEntryNIR>();
    ...
    private float yMin(BubbleEntryNIR entry) {
        return entry.getVal();
    }

    private float yMax(BubbleEntryNIR entry) {
        return entry.getVal();
    }

    private float xMin(BubbleEntryNIR entry) {
        return (float) entry.getXIndex();
    }

    private float xMax(BubbleEntryNIR entry) {
        return (float) entry.getXIndex();
    }

    private float largestSize(BubbleEntryNIR entry) {
