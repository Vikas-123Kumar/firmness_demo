...
package com.github.mikephil.chartingNIR.dataNIR.filter;
...
public class Approximator {
     ...
     * @return
     */
    public List<EntryNIR> filter(List<EntryNIR> points) {
     ...
     * @return
     */
    public List<EntryNIR> filter(List<EntryNIR> points, double tolerance) {
     ...
     * @return
     */
    private List<EntryNIR> reduceWithDouglasPeuker(List<EntryNIR> entries, double epsilon) {
        ...
        List<EntryNIR> reducedEntries = new ArrayList<EntryNIR>();
        for (int i = 0; i < entries.size(); i++) {
            if (keep[i]) {
                EntryNIR curEntryNIR = entries.get(i);
                reducedEntries.add(new EntryNIR(curEntryNIR.getVal(), curEntryNIR.getXIndex()));
            }
        }
    ...
    /**
     * apply the Douglas-Peucker-Reduction to an List of EntryNIR with a given
     * epsilon (tolerance)
     * 
     * @param entries
     ...
     * @param end
     */
    private void algorithmDouglasPeucker(List<EntryNIR> entries, double epsilon, int start,
                                         int end) {
        if (end <= start + 1) {
            // recursion finished
            return;
        ...
        EntryNIR firstEntryNIR = entries.get(start);
        EntryNIR lastEntryNIR = entries.get(end);

        for (int i = start + 1; i < end; i++) {
            double dist = calcAngleBetweenLines(firstEntryNIR, lastEntryNIR, firstEntryNIR, entries.get(i));
     ...
     * (point)
     * 
     * @param startEntryNIR line startpoint
     * @param endEntryNIR line endpoint
     * @param entryNIRPoint the point to which the distance is measured from the
     *            line
     * @return
     */
    public double calcPointToLineDistance(EntryNIR startEntryNIR, EntryNIR endEntryNIR, EntryNIR entryNIRPoint) {

        float xDiffEndStart = (float) endEntryNIR.getXIndex() - (float) startEntryNIR.getXIndex();
        float xDiffEntryStart = (float) entryNIRPoint.getXIndex() - (float) startEntryNIR.getXIndex();

        double normalLength = Math.sqrt((xDiffEndStart)
                * (xDiffEndStart)
                + (endEntryNIR.getVal() - startEntryNIR.getVal())
                * (endEntryNIR.getVal() - startEntryNIR.getVal()));
        return Math.abs((xDiffEntryStart)
                * (endEntryNIR.getVal() - startEntryNIR.getVal())
                - (entryNIRPoint.getVal() - startEntryNIR.getVal())
                * (xDiffEndStart))
                / normalLength;
    }

    /**
     * Calculates the angle between two given lines. The provided EntryNIR objects
     ...
     * @return
     */
    public double calcAngleBetweenLines(EntryNIR start1, EntryNIR end1, EntryNIR start2, EntryNIR end2) {
     ...
     * @return
     */
    public double calcAngleWithRatios(EntryNIR p1, EntryNIR p2) {
     ...
     * @return
     */
    public double calcAngle(EntryNIR p1, EntryNIR p2) {
